...
module.exports = function (modules, dirname, port) {
    ...
    var error = path.join(dirname, "..", "modules", "..", modules.errorHandler.module, "..", "index.js"); // get the error handler
    http
        ...
        res.download = function (status, headers, file, filename) {
            if (filename) {
                res.setHeader(status, "Content-Disposition", "attachment; filename=\"" + filename + "\"", headers);
            }
            else {
